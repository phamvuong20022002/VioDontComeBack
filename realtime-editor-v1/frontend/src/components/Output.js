...
import {
  getCodeWithSocket,
  generateCode,
} from "../helpers/CodeSelectedTabs.js";
import LoadingSpinner from "./LoadingSpinner.js";
import { AppContext } from "../contexts/main_context/index.js";
import { createFilesAndZip } from "../helpers/DownloadCode.js";

const initHTMLContent =
  "<html><head></head><body>Hello, iframe content!</body></html>";
const Output = ({ socketRef, roomId }) => {
  ...
  const getCodeFromSelectedTabs = async () => {
    try {
      ...
      const data = await getCodeWithSocket(socketRef.current, {
        roomId,
        data: selectedTabs,
  ...
  const addTabsToForm = () => {
    ...
    let tabsArray = [];
    let tabsContainer = Array.from(
      document.getElementsByClassName("tabContainer")
    );
  ...
  async function handleRightIcon(e) {
    ...
    if (e.target) {
      ...
      //export icon button
        ...
        // console.log("code export::", data);
        createFilesAndZip(data);
      }
      //new window icon button
      else if (e.target.id === "newWindow-icon") {
        const newWindow = window.open(
          `${getIframeSrc()}?id=${encodeURIComponent(socketRef.current.id)}`
        );
        if (newWindow) {
            ...
            <div className="outputTaskbar">
              ...
              <div className="rightIcon monitor" onClick={handleRightIcon}>
                <VscTerminal
                  className="outputIcon"
                  id="terminal-icon"
                  title="Display console monitor"
                />
                <VscRefresh
                  className="outputIcon"
                  id="reload-icon"
                  title="Reload monitor"
                />
                <VscCloudDownload
                  className="outputIcon"
                  id="export-icon"
